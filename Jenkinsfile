        ...
        stage('Deploy To EC2') {
            steps {
                 ...
                 sshagent(credentials: ['ec2-ssh-key']) {
                    ...
                    docker run -d -p 8000:8000 --name fastapi-app $ECR_REPO:$IMAGE_TAG"
                    '''
                }
...
//             steps {
                // sshagent(credentials: ['ec2-ssh-key']) {
                //     sh '''
                //     ssh -o StrictHostKeyChecking=no ubuntu@13.126.35.40 \
                //     "aws ecr get-login-password --region ap-south-1 | docker login --username AWS --password-stdin 767289848364.dkr.ecr.ap-south-1.amazonaws.com && \
                //     docker stop fastapi-app || true && \
                //     docker rm fastapi-app || true && \
                //     docker pull 767289848364.dkr.ecr.ap-south-1.amazonaws.com/devops-fastapi:latest && \
                //     docker run -d -p 8000:8000 --name fastapi-app 767289848364.dkr.ecr.ap-south-1.amazonaws.com/devops-fastapi:latest"
                //     '''
                // }
//             }
//         }
